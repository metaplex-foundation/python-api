...
class MetaplexAPI():
    ...
    def deploy(self, api_endpoint, name, symbol, max_retries=3, skip_confirmation=False, max_timeout=60, target=20, finalized=True, fee=500):
        """
        ...
        """
        try:
            tx, signers, contract = deploy(api_endpoint, self.account, name, symbol, fee=fee)
            print(contract)
            resp = execute(
                api_endpoint,
                tx,
